...
// assets
import { BorderOutlined, DownOutlined, RightOutlined, CaretUpOutlined, CaretDownOutlined } from '@ant-design/icons';
...
const NavCollapse = ({ menu, level, parentId, setSelectedItems, selectedItems, setSelectedLevel, selectedLevel }) => {
  ...
  const classNameIcon = menu.classNameIcon;
        ...
        <>
          ...
          >
            {menuIcon && (
              ...
              >
                {classNameIcon ? <i className={Icon + ' ottr-icon'} style={{fontSize: drawerOpen ? '1.1rem' : '1.25rem'}}></i> : menuIcon}
              </ListItemIcon>
            )}
            {(drawerOpen || (!drawerOpen && level !== 1)) && (
              <ListItemText
                primary={
                  <Typography id={menu.id} variant="h6" color={selected === menu.id ? 'primary' : textColor}>
                    {menu.title}
                  </Typography>
                }
                secondary={
            ...
            {(drawerOpen || (!drawerOpen && level !== 1)) &&
              (openMini || open ? (
                <CaretUpOutlined style={{ fontSize: '0.625rem', marginLeft: 1, color:'#5E6170', display: Icon === 'icon-dashboard' ? 'none' : '' }} />
              ) : (
                <CaretDownOutlined style={{ fontSize: '0.625rem', marginLeft: 1, color:'#5E6170', display: Icon === 'icon-dashboard' ? 'none' : '' }} />
              ))}
            ...
            {!drawerOpen && (
              <PopperStyled
                ...
                style={{
                  zIndex: menu.id === 'dashboard' ? 1 : 2001
                }}
                popperOptions={{
                  modifiers: [
                    {
        ...
        <>
          ...
          >
            <Box onClick={handlerIconLink} sx={FlexBox}>
              ...
              <ListItemText
                primary={
                  <Typography id={menu.id} variant="body1" color="inherit" sx={{ my: 'auto' }}>
                    {menu.title}
                  </Typography>
                }
              />
