...
export function GlobalFilter({ searchId, preGlobalFilteredRows, globalFilter, setGlobalFilter, ...other }) {
  const count = preGlobalFilteredRows.length;
  // const [value, setValue] = useState(globalFilter);

  const dispatch = useDispatch();
  // Get search flag from global store
  ...
  return (
    <OutlinedInput
      ...
      placeholder={intl.formatMessage({ id: 'PlaceholderRecordscount' }, { count: count })}
      id={searchId}
      // id="start-adornment-email"
      startAdornment={<i className='icon-search ottr-icon' style={{ marginTop: '5px', fontSize: '22px', color: '#919398' }}></i>}
      endAdornment={<button hidden={!search?.length} style={{border: 'none', outline: 'none', backgroundColor: 'transparent', color: 'inherit', marginRight: -10, marginTop: 6, cursor: 'pointer', position: 'absolute', right: '15px'}} onClick={removeSearchedText}><i className='icon-close ottr-icon' style={{ marginTop: '5px', fontSize: '16px', color: '#919398' }}></i></button>}
      {...other}
      sx={{
        '&.Mui-focused': {
          boxShadow: 'none',
    ...
    <FormControl sx={{ width: '100%' }}>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <DatePicker
          ...
          renderInput={(params) => <TextField name={Header} {...params} placeholder={intl.formatMessage({ id: 'PlaceholderRecordscount' }, { header: Header })} />}
        />
