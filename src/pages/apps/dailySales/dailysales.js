...
// Services
import { getDailySaleReport } from '_api/reports/report';
...
function ReactTable({ columns, data, getHeaderProps, date, setDate, setUpdated }) {
  ...
  const getData = () => {
    if (data.length) {
      ...
      const combinedData = [
        ...
        {
          "date": intl.formatMessage({ id: 'TotalAmount' }),
  ...
  const totalAmount = calculateAndRoundTotal(data, 'totalAmount');
  ...
  return (
    ...
    <Stack spacing={3} className='table-filter'>
            ...
            <Stack direction={matchDownSM ? 'column' : 'row'} alignItems="center" spacing={1}>
              <Stack spacing={0.5} sx={{ pt: "17px" }}>
                <CSVExport
                  headers={csvHeaders}
                  data={csvExportData}
                  filename={`${intl.formatMessage({ id: 'Dailysale' })} from ${moment(date.fromDate).local().format("DD-MM-YYYY")} to ${moment(date.toDate).local().format("DD-MM-YYYY")}.csv`}
                  getData={getData}
                  excelRef={excelRef}
                  isApiCallRequired={true}
                  idCSVExport="daily-sale-export-btn"
          ...
          <TableWrapper id='daily-sale-table'>
              ...
              <TableHead>
                {headerGroups.map((headerGroup, i) => (
                  <TableRow
                    ...
                    sx={{ '& > th:first-of-type': { width: '280px' } }}
                  >
...
const CustomCell = ({ row, value }) => {
  ...
  return (
    <Stack direction="row" spacing={1.5} alignItems="center">
        <Stack spacing={0} >
        <Typography variant="subtitle1" pl={values[value] == 0 || values[value] == null ? 6 : 0}>{values[value] == null ? '-' : value === 'totalAmount' ? values[value]?.toFixed(2) : values[value]}</Typography>
        </Stack>
    </Stack>
  );
};
...
const DailySale = () => {
  ...
  const { isReportLoading, dailySaleReports } = useSelector((state) => state.reportSlice);
  ...
  useEffect(() => {
    setFilteredReportData(dailySaleReports);
  }, [dailySaleReports]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => <CustomCell row={row} value='totalAmount' />
