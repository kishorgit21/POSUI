...
import { getVendorWiseExpiredStockReport } from '_api/reports/report';
...
function ReactTable({ columns, data, getHeaderProps, vendors, vendor, setVendor, date, setDate, setUpdated }) {
  ...
  // const sortBy = [{ id: 'expiryDate', desc: false }];
  ...
  const csvHeaders = columns.map((column) => {
    ...
    const key = column.subAccessor && column.subAccessor === 'expiryDate'
        ? column.subAccessor
        : column.accessor;
    return { label, key };
  });
  ...
  const getData = () => {
    if (data.length) {
      const updatedList = data.map((val) => {
        const obj = {
          ...val,
          expiryDate: moment(val.expiryDate).local().format("DD/MM/YYYY")
        }
        return obj
      })
      SetCSVExportData(updatedList);
  ...
  return (
    ...
    <Stack spacing={3} className='table-filter'>
            ...
            <Stack spacing={0.5}>
              ...
              <Stack spacing={0.5} sx={{ pt: "17px" }}>
                <CSVExport
                  headers={csvHeaders}
                  data={csvExportData}
                  filename={`${intl.formatMessage({ id: 'ExpiredStock' })} for ${moment(date).local().format("DD-MM-YYYY")}.csv`}
                  getData={getData}
                  excelRef={excelRef}
                  isApiCallRequired={true}
                  idCSVExport="vendor-wise-expired-stock-export-btn"
          ...
          <TableWrapper id='vendor-wise-expired-stock-table'>
              ...
              <TableHead>
                {headerGroups.map((headerGroup, i) => (
                  <TableRow
                    ...
                    sx={{ 
                      '& > th:first-of-type': { width: '300px' }, 
                      '& > th:nth-of-type(2)': { width: '300px' },
                      '& > th:last-of-type': { width: '300px' }, 
                      ...
                      <Fragment key={i}>
                        <TableRow
                          ...
                          sx={{
                            cursor: 'pointer',
                            bgcolor: 'inherit',
                            '& > td:last-of-type div': { marginRight: '9px' },
...
// ==============================|| VENDOR WISE EXPIRED STOCK - REPORT LIST ||============================== //
...
/**
 * Vendor wise expired stock report list page
 * @returns
 */
const VendorWiseExpiredStock = () => {
  const theme = useTheme();
  ...
  const { isReportLoading, vendorWiseExpiredStockReports } = useSelector((state) => state.reportSlice);
  ...
  useEffect(() => {
    setFilteredReportData(vendorWiseExpiredStockReports);
  ...
  useEffect(() => {
    if (vendor && vendor.id && isValidDate(date)) {
      ...
      // Set model to get vendor wise expired stock report list
      const model = {
        vendorId: vendor.id === 'all' ? '' : vendor.id,
        date: encodeURIComponent(date.toISOString())
      };

      // Get vendor wise expired stock list api call
      dispatch(getVendorWiseExpiredStockReport({ model }))
      .unwrap()
      .then((payload) => {
        if (payload && payload.isError) {
          // Handle error
  ...
  const columns = useMemo(
    () => [
      ...
      {
          Header: `${intl.formatMessage({ id: 'ExpiryDate' })}`,
          Cell: ({ row }) => <CustomCell row={row} value={intl.formatMessage({ id: 'ExpiryDate' })} />,
          accessor: d => {
              return moment(d.expiryDate)
              .local()
              .format("DD/MM/YYYY")
            },
          // Custom sorting method for the date column
          sortType: (rowA, rowB) => {
            const dateA = new Date(rowA.original.expiryDate);
            const dateB = new Date(rowB.original.expiryDate);
            return dateB - dateA;
          },
          subAccessor: 'expiryDate'
        // displayType: 'none',
      }
    ],
...
export default VendorWiseExpiredStock;
