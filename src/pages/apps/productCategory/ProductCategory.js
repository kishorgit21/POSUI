...
// Services
import { getProductCategory, deleteProductCategory, revokeDeletedProductCategory } from '_api/master/product_Category'
...
// Propmt components
import AddProductCategory from 'sections/apps/productCategory/AddProductCategory';
import AlertProductCategoryDelete from 'sections/apps/productCategory/AlertProductCategoryDelete';
...
function ReactTable({ columns, data, getHeaderProps, handleAdd, setShowDelete, showDelete, deletelable, AddProductCategory }) {
  const theme = useTheme();
  const matchDownSM = useMediaQuery(theme.breakpoints.down('sm'));

  const filterTypes = useMemo(() => renderFilterTypes, []);

  // Sorting product category list parameters
      ...
      <Stack spacing={3} className='table-filter' >
        ...
        >
          <GlobalFilter
            ...
            searchId="product-category-search"
          />
          <Stack direction={matchDownSM ? 'column' : 'row'} alignItems="center" spacing={1}>
            <FormControlLabel
              ...
              control={<Switch id="product-category-deleted-switch" color="primary" style={{ pointerEvents: "auto" }} />}
              label={deletelable}
              labelPlacement="start"
              sx={{ mr: 1 }}
            />
            <Button  id="add-product-category-btn" variant="outlined" className="btn-outlined-primary add-product" endIcon={<i className='icon-plus  ottr-icon'></i>} onClick={() => handleAdd('add')} size="large">
              {AddProductCategory}
            </Button>
            <CSVExport
              ...
              filename={`${intl.formatMessage({ id: 'productCategory' })}-${intl.formatMessage({ id: 'list' })}.csv`}
              isApiCallRequired={false}
              idCSVExport="product-category-export-btn"
              isDisabled={data.length ? false : true}
            />
          </Stack>
        </Stack>
        <ScrollX sx={{ maxHeight: 'calc(100vh - 340px)' }} className='ottr-table'>
          <TableWrapper id='product-categories-table'>
...
const ActionCell = (row, setProductCategory, setProductCategoryDeleteId, handleClose, handleAdd, revokeDeletedItem) => {
  const intl = useIntl();
    ...
    <Stack direction="row" alignItems="center" justifyContent="center" spacing={0} className='action-icons'>
      ...
      {row.values && row.values.recordStatus === 0 ? (
        <Tooltip title={intl.formatMessage({ id: 'Delete' })}>
          <IconButton
            id="delete-product-category-icon"
            color="error"
            onClick={(e) => {
              e.stopPropagation();
              setProductCategoryDeleteId(row.values);
              handleClose();
            }}
          >
      ...
      {row.values && row.values.recordStatus === 1 && (
        <Button id="product-category-restore-btn" className="btn-outlined-primary add-product" sx={{ marginRight: "-5px !important" , padding:"0 5px !important" }} variant="outlined" onClick={() => revokeDeletedItem(row.values)} size="small">
...
const ProductCategory = () => {
  ...
  // Set add product category state
  const [add, setAdd] = useState(false);

  // Set add/edit product category status state
  const [addEditStatus, setAddEditStatus] = useState('');

  // Set open prompt state
  const [open, setOpen] = useState(false);

  // Set selected product category state
  const [productCategory, setProductCategory] = useState();

  // Set product category delete id state
  const [productCategoryDeleteId, setProductCategoryDeleteId] = useState();
  ...
  const handleClose = (isOpen, status) => {
    if (status === 'delete') {
      // Set delete model
      const model = {
        id: productCategoryDeleteId.id
      };

      // Delete product category api call
      dispatch(deleteProductCategory({ model }))
        .unwrap()
        .then((payload) => {
          ...
          if (payload && payload.isError) {
            ...
            dispatch(
              openSnackbar({
                open: true,
                message: `${intl.formatMessage({ id: 'MasterProductCategoryTostDelete' })}`,
                variant: 'alert',
                alert: {
                  color: 'success'
                },
        ...
        .catch((error) => {
          if (error && error.code === 'ERR_BAD_REQUEST') {
            dispatch(
              openSnackbar({
                open: true,
                message: `${intl.formatMessage({ id: 'MasterProductCategoryDeleteErrorMsg' })}`,
                variant: 'alert',
                alert: {
                  color: 'error'
                },
  ...
  const revokeDeletedItem = (deletedItem) => {
    ...
    dispatch(revokeDeletedProductCategory({ model }))
      .unwrap()
      .then((payload) => {
        // Check for error & success
        if (payload && payload.isError) {
          ...
          dispatch(
            openSnackbar({
              open: true,
              message: `${intl.formatMessage({ id: 'MasterProductCategoryTostRevoke' })}`,
              variant: 'alert',
              alert: {
                color: 'success'
              },
      ...
      .catch((error) => {
        if (error && error.code === 'ERR_BAD_REQUEST') {
          dispatch(
            openSnackbar({
              open: true,
              message: `${intl.formatMessage({ id: 'MasterProductCategoryRevokeErrorMsg' })}`,
              variant: 'alert',
              alert: {
                color: 'error'
              },
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => ActionCell(row, setProductCategory, setProductCategoryDeleteId, handleClose, handleAdd, revokeDeletedItem, theme)
      }
    ],
  ...
  // Get product category list, loading flag & another parameters
  const { isLoading, productCategories } = useSelector((state) => state.productCategorySlice);
  ...
  useEffect(() => {
    const data = isDeleted ? productCategories : productCategories.filter((item) => item.recordStatus === 0);
  ...
  }, [isDeleted, productCategories, search]);

  const dispatch = useDispatch();

  useEffect(() => {
    // Get product category list api call
      ...
      .catch((error) => {
        if (error && error.code === 'ERR_BAD_REQUEST') {
          dispatch(
            openSnackbar({
              open: true,
              message: `${intl.formatMessage({ id: 'MasterProductCategoryListErrorMsg' })}`,
              variant: 'alert',
              alert: {
                color: 'error'
              },
      ...
      <MainCard content={false} className="ottr-table-section">
        <ScrollX>
          <ReactTable
            ...
            AddProductCategory={intl.formatMessage({ id: 'AddProductCategory' })}
          />
        </ScrollX>
        {open && <AlertProductCategoryDelete title={productCategoryDeleteId.name} open={open} handleClose={handleClose} />}
        {/* add user dialog */}
        {add && (
          <Dialog
          ...
          >
            <AddProductCategory addEditStatus={addEditStatus} productCategory={productCategory} onCancel={handleAdd} setUpdated={setUpdated} />
          </Dialog>
        )}
      </MainCard>
    </>
