      ...
      {!column.disableSortBy && (
        <Stack sx={{ color: 'secondary.light' }} {...(sort && { ...column.getHeaderProps(column.getSortByToggleProps()) })}>
          <CaretUpOutlined
            style={{
              fontSize: '0.625rem',
              color: column.isSorted && !column.isSortedDesc ? "#000000" : 'inherit'
            }}
          />
          <CaretDownOutlined
            style={{
              fontSize: '0.625rem',
              marginTop: -2,
              color: column.isSortedDesc ? "#000000" : 'inherit'
            }}
          />
        </Stack>
      )}
...
export const TablePagination = ({ gotoPage, rows, setPageSize, pageSize, pageIndex, isDisabled }) => {
  ...
  return (
    <Grid container alignItems="center" justifyContent="space-between" sx={{ width: 'auto' }} className='table-pagination'>
      <Grid item>
        <Stack direction="row" spacing={1} alignItems="center">
          <Stack direction="row" spacing={1} alignItems="center">
            <Typography variant="caption" color="secondary" className="row-per-page">
          ...
          <Typography variant="caption" color="secondary" className='go-to'>
...
export const CSVExport = ({ data, filename, headers, getData, excelRef, isApiCallRequired,idCSVExport, isDisabled }) => {
  const intl = useIntl();
  return (
    isApiCallRequired ?
    <>
    <Button className="csv-btn" variant="outlined" onClick={getData} id={idCSVExport} disabled={isDisabled}>
        <Tooltip title={intl.formatMessage({ id: 'CSVExportLabel' })}>
    ...
    {data.length ? <CSVLink data={data} filename={filename} headers={headers} ref={excelRef} disabled={!data.length ? true : false}></CSVLink> : '' }
    </>
    : isDisabled ?
    <Button className="csv-btn" variant="outlined" id={idCSVExport} disabled={isDisabled}>
        <Tooltip title={intl.formatMessage({ id: 'CSVExportLabel' })}>
          {/* <DownloadOutlined style={{ fontSize: '24px', color: 'gray', marginTop: 4, marginRight: 4, marginLeft: 4 }} /> */}
          <i className='icon-download ottr-icon'></i>
        </Tooltip>
    ...
    <CSVLink data={data} filename={filename} headers={headers}>
      <Button className="csv-btn" variant="outlined" id={idCSVExport}>
        <Tooltip title={intl.formatMessage({ id: 'CSVExportLabel' })}>
          {/* <DownloadOutlined style={{ fontSize: '24px', color: 'gray', marginTop: 4, marginRight: 4, marginLeft: 4 }} /> */}
          <i className='icon-download ottr-icon'></i>
        </Tooltip>
